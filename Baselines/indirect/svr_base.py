...
def fit_svr(args):
	"""
	function to fit cross-validated support-vector-regression
	"""
	...
	pipe = Pipeline([('standardscaler', StandardScaler()), ('svr',SVR(kernel='rbf'))])

	parameters = {'svr__C':[1, 2], 'svr__epsilon':[.01]} #'svr__kernel':('linear','rbf'), 
...
def fit_svr_repeat(args):
	"""
	function to fit cross-validated support-vector-regression multiple times
	"""
	if args.verbose:
		start = time.time()
		print(f'Start treatment model fitting: size: {args.n_train+args.n_val}')
	
	res = np.array([fit_svr(args) for i in range(args.n_repeats)])
